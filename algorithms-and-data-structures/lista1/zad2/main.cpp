...
void testMerge() {
    ...
    LinkedList<int> mergedLists(list1, list2);

    std::cout << "Merged lists:        " << mergedLists << std::endl;
    std::cout << "List 1 after merge:  " << list1 << "\nList 2 after merge:  " << list2 << std::endl;
}

double meanTimeToAccessElement(const LinkedList<int>& list, int elementIndex) {
...
double meanTimeToAccessRandomElement(const LinkedList<int>& list, int minElementIndex, int maxElementIndex) {
    constexpr int triesNumber = 1000;
